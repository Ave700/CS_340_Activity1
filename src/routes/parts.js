...
/**
 * Logic for actually adding a new part using data from a form submission.
 */
router.post('/parts/add', (req, res, next) => {
    let context = createViewContext();

    // Make sure a supplier with the provided SID doesn't already exist
    req.db.query('SELECT * FROM Parts WHERE pid = ?', [req.body.sid], (err, results) => {
        if (err) return next(err);

        if (results.length) {
            // Already exists
            context.message = `Can't create supplier with SID ${req.body.pid} because it already exists`;
